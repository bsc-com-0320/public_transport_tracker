...
class RideFilterOptions {
  final double
  pickupRadiusKm; // Radius in kilometers for filtering rides near the user's pickup point.
  final double
  dropoffRadiusKm; // Radius in kilometers for filtering rides near the user's dropoff point.
  final RideSortCriteria
  sortCriteria; // The criteria to use for sorting the filtered rides.
  ...
  const RideFilterOptions({
    ...
    this.sortCriteria =
        RideSortCriteria
            .shortestDistanceFromPickup, // Default sort by shortest distance.
  });
  ...
  static RideSortCriteria _parseSortCriteria(dynamic criteria) {
    ...
    return RideSortCriteria
        .shortestDistanceFromPickup; // Default if not found or invalid
  }
}
...
class RideSearchService {
  ...
  List<Map<String, dynamic>> searchRides({
    ...
    for (var ride in allRides) {
      ...
      final LatLng? ridePickupLatLng = _parseLatLng(
        ride['pickup_latitude'],
        ride['pickup_longitude'],
      );
      final LatLng? rideDropoffLatLng = _parseLatLng(
      ...
      final double distanceToDropoff = calculateDistance(
        userDropoff,
        rideDropoffLatLng,
      );
    ...
    filteredRides.sort((a, b) {
      // Corrected from filteredRades to filteredRides
      switch (options.sortCriteria) {
        case RideSortCriteria.shortestDistanceFromPickup:
          ...
          final double distA =
              a['calculated_distance_to_pickup'] ?? double.infinity;
          final double distB =
              b['calculated_distance_to_pickup'] ?? double.infinity;
          return distA.compareTo(distB);
        case RideSortCriteria.lowestCost:
          // Sort by the total cost of the ride
          final double costA =
              (a['total_cost'] as num?)?.toDouble() ?? double.infinity;
          final double costB =
              (b['total_cost'] as num?)?.toDouble() ?? double.infinity;
          return costA.compareTo(costB);
        case RideSortCriteria.earliestDeparture:
          // Sort by the departure time
          final DateTime timeA = _parseDepartureTime(a['departure_time']);
          final DateTime timeB = _parseDepartureTime(b['departure_time']);
  ...
  // Helper to parse departure time string to DateTime.
